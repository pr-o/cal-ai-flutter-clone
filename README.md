...
│   └── nutritionix_service.dart # 음식 텍스트 검색 API 호출
├── utils/
...
```
SearchScreen → NutritionixService.searchFoods()
...
- **인증:** `x-app-id` + `x-app-key` 헤더 (SharedPreferences에서 읽어옴)
- **입력:** `{ query: "삶은 달걀 2개와 토스트" }`

두 API 키 모두 설정 화면에서 사용자가 한 번 입력하면 SharedPreferences에 저장된다.
...
│   └── nutritionix_service.dart # Food text search API calls
├── utils/
...
- **Auth:** `x-app-id` + `x-app-key` headers (read from SharedPreferences)
- **Input:** `{ query: "2 boiled eggs and toast" }`

Both API keys are entered once by the user in Settings and stored in SharedPreferences.
